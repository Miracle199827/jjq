<template>
	<view class="integratedQuery_content">
		<view class="search">
			<view class="example">
			...
			getIntegratedQuery() {
				uni.request({
					...
					success:(res)=>{
						...
						this.IntegratedQuery = res.data.integratedQuery
					},
					fail: (err) => {
...
<style scoped>
	...
	.integratedQuery_items {
		display: flex;
		flex-direction: column;
		background-color: #fff;
	}

	.integratedQuery_items view {
		display: flex;
		flex: 1;
		justify-content: flex-start;
		flex-direction: column;
	...
	.integratedQuery_items view text {
	    margin-bottom: 2rpx;
	    font-size: 26rpx;
	    color: #999999;
	}
	
	.integratedQuery_items view .items_title {
	    font-size: 30rpx;
	    color: black;
	}
</style>
